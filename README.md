...
- What a pattern says by looking back or ahead. The checker settles the text of a `String.matches` pattern, and this
  compiles that text into a machine before the program runs rather than at run time, and what it is read into holds every step the walk could be at rather than
